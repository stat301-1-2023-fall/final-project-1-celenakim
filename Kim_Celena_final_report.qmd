...
#| fig-cap: The disribution of opening weekend revenue for movies that have one at least won Oscar award and movies who have won 0 Oscars.
movie_data |> 
  filter(!is.na(oscar_winners)) |> 
...
year_dom_gross <- movie_data |> 
  group_by(year) |> 
  summarize(mean_dom_gross = round(mean(domestic_gross_million, 
                                        na.rm = TRUE)))  |> 
...
gross_dom_yearly
```
@fig-yearly-dom-gross visualizes the change in the yearly average domestic gross (in millions) for Hollywood movies from 2007-2022. Just as in @fig-yearly-opening-wknd, the lowest point on this graph also corresponds to the year 2020, revealing that in addition to opening weekend revenue, domestic gross was also affected by the 2020 COVID-19 pandemic. With the closing of movie theaters due to quarantine during that year, the average domestic gross was significantly lower than compared to other years. Like opening weekend revenue, the domestic gross for 2020 was greatly impacted by the pandemic.
